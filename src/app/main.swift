...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private var downloadWallpapersStatusItem: NSStatusItem?
    ...
    private func applyDisplayMode() {
        ...
        downloadWallpapersStatusItem = nil
        ...
        NSStatusBar.system.removeStatusItemIfNeeded(downloadWallpapersStatusItem)
    ...
    private func setupSingleStatusItem() {
        ...
        if let button = item.button {
            button.image = NSImage(systemSymbolName: "tray", accessibilityDescription: "Macos PKScriptsUtilities")
            button.toolTip = "Macos PKScriptsUtilities"
        }
        ...
        let downloadWallpapersItem = NSMenuItem(title: "Download Wallpapers", action: #selector(runDownloadWallpapers), keyEquivalent: "")
        downloadWallpapersItem.image = NSImage(systemSymbolName: "photo.stack", accessibilityDescription: nil)
        menu.addItem(downloadWallpapersItem)
    ...
    private func setupMultiStatusItems() {
        ...
        let downloadWallpapersItem = NSStatusBar.system.statusItem(withLength: NSStatusItem.variableLength)
        if let button = downloadWallpapersItem.button {
            button.image = NSImage(systemSymbolName: "photo.stack", accessibilityDescription: "Download Wallpapers")
            button.target = self
            button.action = #selector(runDownloadWallpapers)
            button.toolTip = "Download Wallpapers"
        }
        downloadWallpapersStatusItem = downloadWallpapersItem
    ...
    @objc private func runArchive() {
        _ = runScript("archive.sh")
    }

    @objc private func runDl2desk() {
        _ = runScript("dl2desk.sh", args: ["--once"])
    }

    @objc private func runDownloadWallpapers() {
        _ = runScript("download_wallpapers.sh")
    }
    ...
    @objc private func toggleWallpaperLoop() {
        ...
        _ = runScript("change-wallpaper.sh", args: ["--once"])
        wallpaperProcess = runScript("change-wallpaper.sh")
        updateWallpaperUI()
    }
